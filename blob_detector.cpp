...
// see https://medium.com/image-processing-in-robotics/blob-detection-309226a3ea5b
// iterative algorithm to find connected regions in a binarized image
...
typedef struct pt_{ int x,y;
                  } pt;

typedef struct blob_{ int min_x,max_x;
                      int min_y,max_y;
                      int cen_x,cen_y;
                      int n_pixels;
                      int ID;
...
void GetBlobs (Mat img, vector<blob> & blobs) {
    ...
    int i,j,k,l,r = img.rows,
        c = img.cols,
        id=1;

    vector<vector<int>> pixel_ID(r, vector<int>(c, -1));   //Stores ID of a pixel; -1 means unvisited queue
    ...
    for (i=1; i < r-1; i++) {
        for (j=1; j < c-1; j++) {
             if (img.at<uchar>(i, j) == 0 || pixel_ID[i][j] > -1) continue;
             ...
             while (!open_list.empty()) {
                ...
                for (k=top.y-1; k<=top.y+1; k++) {
                  for (l=top.x-1; l<=top.x+1; l++) {
                    if (img.at<uchar>(k,l) == 0 || pixel_ID[k][l] > -1) continue;
                    pt next = {l,k};
                    pixel_ID[k][l] = id;
                    open_list.push(next);
                  }
    ...
    //To test correctness; can use the vector as desired
}
